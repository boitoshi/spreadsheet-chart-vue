...
### 🔜 次フェーズ（為替損益分離チャート）
- [ ] backend `performance.py`: L-P列（外貨・為替データ）を読み込み、株価損益・為替損益を計算して返す
- [ ] backend `schemas/history.py`: `stockProfit` / `fxProfit` / `currency` フィールドを追加
- [ ] frontend `types/index.ts`: 同フィールドを追加
- [ ] frontend `ProfitAreaChart.tsx`: 積み上げ棒グラフ（株価損益 + 為替損益）に変更

### 📋 高優先度
1. **アセットアロケーション推移チャート** — ダッシュボードの円グラフを月次積み上げ面グラフに拡張
2. **銘柄別パフォーマンス比較** — 損益推移ページで複数銘柄を同一時軸で重ね表示
3. **配当・分配金記録** — スプレッドシートに配当シートを追加しトータルリターン管理

### 📋 低優先度
4. **月次レポートの Web プレビュー** — `output/blog_draft_*.md` をブラウザ上でプレビュー表示

### 📋 中優先度
5. **CAGR（年率換算リターン）** — 取得日からの保有期間を考慮した年率リターン表示
6. **ベンチマーク比較** — 日経225 / S&P500 と自ポートフォリオのリターンを並べて表示
7. **通貨エクスポージャーサマリー** — JPY/USD/HKD ごとの評価額・損益率を一覧表示
