...
### 残っているもの → **無し**（2026-08-05 時点）

- ~~配当データがまだ入っていない~~ … **2026-08-05 ユーザー確認により入力完了**。
  `--add-dividend` での随時入力運用に乗った。空ページ状態は解消
- したがって移送時に想定した残作業は消化済み。ユーザーが「修正」と言っていた
  具体的な不具合が上記3項目以外にあるなら、ここに追記する
